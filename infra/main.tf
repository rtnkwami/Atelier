...
locals {
  ...
  database_url       = "postgresql://${data.aws_ssm_parameter.database_user.value}:${data.aws_secretsmanager_secret_version.database_password.secret_string}@${aws_db_instance.db_primary.endpoint}/${data.aws_ssm_parameter.database_name.value}"
}